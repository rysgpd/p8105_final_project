...
# Step 1: Create a lookup table with unique player names and their IDs
...
# Step 2: Update the `winner_id` and `loser_id` columns in the original dataset
...
# Step 3: Convert IDs back to integers if needed
cleaned_df_updated_2 <- cleaned_df_updated_2 %>%
...
# Step 1: Scrape data from the US Open website
url <- "https://en.wikipedia.org/wiki/2024_US_Open_–_Men%27s_singles"
dl_data <- read_html(url) %>% 
  html_nodes("dl") %>% 
  html_text(trim = TRUE)

# Step 2: Process the scraped data into a clean table
us_open_seed_table <- str_split(dl_data, "\n") %>%  # Split by lines
  ...
  filter(!is.na(Seed)) %>%                                  # Keep only rows with seeds
  select(Seed, Player_Info)                                 # Keep only relevant columns

# View the US Open seed table
...
# Match seeds and update the specific rows
rows_to_update <- rows_to_update %>%
  # Join with the US Open seed table based on winner_name
  left_join(us_open_seed_table, by = c("winner_name" = "Player_Info")) %>%
  mutate(winner_seed = coalesce(as.character(Seed), as.character(winner_seed))) %>%
  select(-Seed) %>%
  # Join with the US Open seed table based on loser_name
...
# Step 1: Scrape data from the French Open website
...
# View the French Open seed table
print(french_open_seed_table)

# Step 3: Update the dataset `cleaned_df_updated_2`
library(dplyr)
...
# Match seeds and update the specific rows
rows_to_update <- rows_to_update %>%
  # Join with the French Open seed table based on winner_name
  left_join(french_open_seed_table, by = c("winner_name" = "Player_Info")) %>%
  mutate(winner_seed = coalesce(as.character(Seed), as.character(winner_seed))) %>%
  select(-Seed) %>%
  # Join with the French Open seed table based on loser_name
...
# Create a lookup table for winner_name and their country codes
winner_country_lookup <- cleaned_df_updated_2 %>%
  filter(!is.na(winner_ioc)) %>%  # Keep only rows with known country codes
  select(winner_name, winner_ioc) %>%
  distinct()                      # Remove duplicates

# Create a lookup table for loser_name and their country codes
loser_country_lookup <- cleaned_df_updated_2 %>%
  filter(!is.na(loser_ioc)) %>%   # Keep only rows with known country codes
  select(loser_name, loser_ioc) %>%
  distinct()                      # Remove duplicates

# Fill in missing country codes for winner_ioc
cleaned_df_updated_2 <- cleaned_df_updated_2 %>%
...
# Fill in missing country codes for loser_ioc
cleaned_df_updated_2 <- cleaned_df_updated_2 %>%
...
```{r}
# Ensure that full_names_for_wimbledon has the necessary columns
# full_names_for_wimbledon should have "full_name" and "country_code"

# Update winner_ioc based on matching winner_name to full_name
cleaned_df_updated_2 <- cleaned_df_updated_2 %>%
  ...
  select(-country_code)  # Remove the temporary country_code column

# Update loser_ioc based on matching loser_name to full_name
cleaned_df_updated_2 <- cleaned_df_updated_2 %>%
...
write_csv(cleaned_df_updated_2, "cleaned_data_with_full_2024_data_added.csv")

# Confirm by viewing the updated dataset
#View(cleaned_df_updated_2)
```
