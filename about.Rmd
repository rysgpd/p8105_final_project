...
Due to well-known tournaments, emerging talent, and coverage of controversies by popular media, tennis has become increasingly popular. We aim to understand the trends of men’s tennis matches at the four major championships (Australian Open, French Open, Wimbledon, and US Open) to provide insights into this dynamic and evolving sport, catering to fans, analysts, and industry stakeholders, especially with the US Open in New York. With prominent players getting suspensions for doping, we also wanted to understand the process more and attempt to track players who had in the past gotten sanctioned. 

## Related Work

Our team was inspired by Jeff Sackman's [Match Charting Project](https://www.tennisabstract.com/charting/meta.html), which many broadcasters and analysts rely on for understanding pro matches. Often, metrics and demographic information is shown during and after matches to understand player performance. Our project aims to further expand on this work and visualize player and match information for a lay audience who may not be familiar with tennis.
...
* Supplementary Match Data
  * The dataset only included data up to the Australian Open which took place in January of 2024. We used web scraping methods to add data for Wimbledon, Roland Garros, and the US Open of 2024 to our dataset. Several challenges were encountered during the web scraping process for additional 2024 data: it was difficult to find tables containing all players with their full names, as most sources only used last names; there were also inconsistencies in name spellings due to the use of accents or lack thereof. Furthermore, tennis game scores were recorded in various formats, which complicated the web scraping process further, for example, sometimes the number of sets was mentioned, sometimes not; tiebreaks were sometimes recorded in parentheses, sometimes not, sometimes dashes were used or only the number of points was mentioned. Additionally, data such as the country of origin or first names for some lesser-known players in the early rounds had to be added manually.

We expanded the dataset to include Roland Garros, Wimbledon, and the US Open 2024 using web scraping and data integration, addressing challenges like inconsistent scores, abbreviated names, and missing seeds or countries, ensuring consistency and completeness for data analysis.

Key steps that were undertaken:

- Data Retrieval: Match data for the additional tournaments was obtained from online sources using read_csv and rvest.
- Data Cleaning: Scores and metadata were standardized using mutate and string processing functions (str_trim, str_replace).
- Resolving Player Names: Full names were matched and replaced using a lookup table created with scraped seed and country data.
- Data Integration: All datasets were merged using bind_rows, and missing details such as seeds and country codes were filled using joins and mutations.
- Assigning Player IDs: IDs were added for players that already had IDs and new unique IDs were generated to ensure consistent identification across entries.
- Final Output: Redundant columns were removed, and the final dataset was saved as a CSV file.

In the following, we can see a web scraping example that retrieves French Open 2024 data, formats the scores and dates, renames the tournament to "Roland Garros," and selects and processes columns for consistency.
...
knitr::kable(head(processed_data, 12))

```

* Suspension/Doping Data
  * A key component of analyzing doping in tennis involved web scraping to gather supplementary data from multiple sources. Since there is no centralized, publicly accessible database for tennis doping cases, we relied on scraping data from sources such as news articles and official sanction announcements on websites like the International Tennis Integrity Agency (ITIA). The creation of the ITIA in 2020 marked a significant shift in how doping cases were reported and recorded as well. 

  * Web scraping introduced challenges, such as extracting structured data from unstructured HTML pages, handling inconsistencies in the reporting format, and dealing with incomplete or ambiguous information. For example, suspension types were often inconsistently reported, requiring verification and alignment with other sources. 
  
  * Problems:
    * The ITIA website's sanctions page lacked a standardized table format, requiring complex scraping logic to extract information from accordion-style elements
    * Player names often appeared in different formats across sources, making it difficult to match them with match data
    * Most players lacked detailed sanction information
    * Merging data from multiple sources (ITIA, Wikipedia, match results) required extensive QA and standardization
    * Some players appeared with different name variations across datasets
    * Sanction durations were often reported inconsistently or incompletely, whereas substances weren’t reported at all
...
- `loser_id.`  
  ...
  - `loser_age`
- `score`  
- `round`: round of the tournament (R128 to F)  
...
Seed rankings play an important role in predicting match outcomes in tennis. This visualization takes a closer look at the number of wins achieved by players of different seed ranks over time. As shown here, a players with higher seeds generally have a higher total number of wins compared to low-seeded players.
...
#### Ages of Winners and Losers

Age is a crucial factor in the game of tennis. The next visualization reveals how the average age of both winners and losers has evolved over time. As the sport becomes more physically demanding, it’s interesting to see if younger or older players are succeeding in these tournaments.
...
For this plot, we counted the number of tournament wins for the top 7 countries across the four Grand Slam tournaments, grouped them by tournament, and displayed the results as a dodged bar chart.
...
#### Surface Matters
The Grand Slam tennis tournaments are played on different surfaces, potentially affecting player performance. The Australian and US Opens use hard courts, the French Open is played on clay, and Wimbledon is played on grass. In this plot, we calculated the number of wins for the top 5 countries on each surface type across Grand Slam tournaments, adjusted for weighting hard court wins by half, and grouped the results by surface.
...
```
For this plot, we calculated the number of wins for the top 5 players at each Grand Slam tournament and grouped the results by tournament.
...
ggplot(filtered_data, aes(x = year, y = participants, fill = country)) +
  ...
  theme_minimal()
```
In this plot, we calculated the yearly number of unique participants from the top 6 countries in Grand Slam tournaments and displayed the trends over time using an area plot.
...
### Match and Player Analyses
There were many analyses done that looked into individual variables (`age`, `seed`) and comparisons between the players in each match. We looked at how seeding affects winning the match, and overwhelmingly (11x more likely) we found that your seed matters a lot because it is a good indication of how well you are performing at that point in time. Seeds are closely related to ATP rankings, which is the overall performance with other tournaments other than grand slams as well. Sometimes, however, there are upsets and as you can see from the different seed types, while most winners are seeded higher, there are still quite a few surprises. There was not an overall grand slam winner from 2014-2024 that was not ranked at least 17th seed though. Age was also an interesting factor to look into. To a regular person who's not a fan of tennis, it would make sense that the older a player is, the worse their performance may be. However, as you can see, older players (such as Djokovic, Nadal, Federer, etc) actually performed a lot better in certain years. Overall, it did not indicate that being older was more likely to win. Younger talent continues to enter every year, but the younger generation struggles to win against the older, fan-favorites at grand slams often.  

We did two regression models: 1) pre-match using established data such as seeding compared to their opponent `seeded_higher`, if they were older than their opponent `older_opponent`, `preferred hand` to use to play, `height`, `age` in general, and `ATP Rank`. We found that the `seeded_higher` and `ATP Rank` which were related to begin with were large indicators, which backed up our previous exploratory statistical analysis. 2) post-match using the match metrics for each players using `Length of Match`, `Aces`, `Double Faults`, `Serve Points`, `1st Serves Made`, `1st Serves Won`, `2nd Serves Won`, `% of Break Points Saved`, `Number of Sets Played`. These are all very common metrics to see on ESPN after each set and match to help the audience see how well the player performed. Many of the significant ones made a lot of sense - the more serves/points you win and save indicate that you're winning the match. Some were interesting to think about such as number of aces, which may indicate that you're relying too much on your serve and being inconsistent with other points. Some were a little confusing and shows future analysis needed to explain like the longer the length of a match and sets played, the more likely you are to win. A limitation is we stuck to a simple linear regression and there are more factors not included in the dataset, such as injuries, player likability, outside influences, etc. 

### Tournament Analyses
The tournament analysis results shows significant trends in player performance, country dominance, and participation patterns over the past decade. Our focus on the Grand Slam tournaments allowed us to gain key insights about geographical dominance, surface-specific performance, player consistency, and rise of new talent.

The analysis of Country Winning the Most shows clear patterns of country-specific dominance. Spanish players, particularly Rafael Nadal, have significantly influenced the results of clay and hard court tournaments. Meanwhile, the USA has maintained a stronghold on grass courts, showcasing how training environments and player development strategies vary across regions.

The Surface Matters section emphasizes how playing surfaces influence player performance and outcomes. It is particularly notable that Spanish players dominate clay courts, reflecting the strong tradition of clay court training in Spain. Grass courts, with their faster pace, continue to favor players from the USA, while hard courts are more evenly contested, though with notable Spanish successes.

The dominance of Novak Djokovic across all Grand Slam tournaments, as seen in the Top Players for Each Tournament section, underlines his almost unparalleled consistency and adaptability. His record-breaking number of wins on multiple surfaces and tournaments cements his place as one of the greatest players in tennis history.

The analysis of Participation Trends Over Time is also noteworthy. The consistent participation of players from France, the USA, and Spain highlights the robust tennis infrastructure in these countries. The dip in 2020 participation, caused by the cancellation of Wimbledon during the COVID-19 pandemic, serves as a reminder of the pandemic’s impact on global sports.

From the analyses looking at trends of winners, we see several key players winning the Grand Slams. The trends of ATP winners over time in Grand Slams visualizations highlights the continued dominance of Novak Djokovic and Rafael Nadal, who consistently win Grand Slam tournaments. This trend highlights their strong performance and consistency in tennis. However, advancements in playing methods and sports science continue to develop and allow athletes to perform at a higher level at a younger age. As seen in the plot, it appears that new and younger players, such as Jannik Sinner, are reshaping the competitive landscape of tennis by challenging the dominance of established champions. 
