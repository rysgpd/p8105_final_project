...
```{r serve, echo = FALSE, message = FALSE, warning = FALSE}
serve_stats <- tennis_data %>%
  mutate(year = format(as.Date(tourney_date), "%Y")) %>%
  group_by(year) %>%
  summarize(total_aces = sum(w_ace, na.rm = TRUE))

ggplot(serve_stats, aes(x = year, y = total_aces)) +
  geom_bar(stat = "identity", fill = "darkblue") +
  labs(title = "Aces per Year in Grand Slams",
       x = "Year", y = "Total Aces") +
  theme_minimal()
```

### Match Duration by Round
```{r, echo = FALSE, message = FALSE, warning = FALSE}
slam_comparison <- tennis_data %>%
  mutate(year = format(as.Date(tourney_date), "%Y"),
         tournament = tourney_name) %>%
  filter(tournament %in% c("Australian Open", "Wimbledon", "US Open", "Roland Garros"))

ggplot(slam_comparison, aes(x = round, y = minutes, color = tournament)) +
  geom_point(alpha = 0.7) +
  facet_wrap(~ tournament) +
  labs(title = "Match Duration by Round for Each Grand Slam",
       x = "Round", y = "Minutes") +
  theme_minimal()
```
